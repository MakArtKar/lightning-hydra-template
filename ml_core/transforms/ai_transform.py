...
class AITransformWrapper(WrapTransform):
    ...
    This class takes a text description of a function, generates Python code using an LLM,
    saves it to a file, and wraps it as a transform.

    :param transform_prompt: Text description of the function to generate. Should start with "def my_function_name".
    ...
    def __init__(
        ...
        if path is None:
            ...
            full_path = Path(path) if Path(path).is_absolute() else PROJECT_ROOT / "ml_core" / "transforms" / path
    ...
    def _generate_code(self, transform_prompt: str, function_name: str, file_path: Path, model: str) -> None:
        ...
        try:
            ...
            raise ImportError("openai package is required for AITransformWrapper. Install with: pip install openai")
        ...
        try:
            response = client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "user", "content": TEMPLATE_PROMPT.format(
    ...
    def _import_transform(self, file_path: Path, function_name: str) -> Callable:
        ...
        try:
            ...
            if not hasattr(module, function_name):
                raise AttributeError(f"Generated module {module_name} does not have a '{function_name}' function")
